...
predictor_up:
	...
	bash deployment/deploy.sh run_predictor data/model_config/phase-1/prob-1/model-2.yaml 5040
...
predictor_curl:
	...
	curl -X POST http://localhost:5040/phase-1/prob-2/predict -H "Content-Type: application/json" -d @data/curl/phase-1/prob-1/payload-1.json
	curl -X POST http://localhost:5040/phase-1/prob-2/predict -H "Content-Type: application/json" -d @data/curl/phase-1/prob-1/payload-2.json
